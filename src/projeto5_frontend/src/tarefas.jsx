...
import { projeto5_backend } from 'declarations/projeto5_backend';
...
function tarefas() {
  ...
  async function consultarTarefas() {
    ...
    try {
      setTarefas(await projeto5_backend.getTarefas());
    } catch (error) {
  ...
  async function buscarTotais() {
    try {
      const totalAndamento = parseInt(await projeto5_backend.totalTarefasEmAndamento()); // Utilização da dica dada pelo professor
      const totalConcluido = parseInt(await projeto5_backend.totalTarefasConcluidas());
   ...
   async function handleSubmit(event) {
    ...
    try {
      if (idTarefa === null || idTarefa === "") {
        await projeto5_backend.addTarefa(descricao, categoria, false, false);
        setMensagemSucesso('Tarefa adicionada com sucesso!');
      } else {
        await projeto5_backend.alterarTarefa(parseInt(idTarefa), categoria, descricao, urgente, false);
 ...
 /* async function excluir(id) {
    setCarregando(true); // Ativa o spinner
    try {
      await projeto5_backend.excluirTarefa(parseInt(id));
      await consultarTarefas(); // Atualiza a lista de tarefas
    } catch (error) {
...
*/
  ...
  const confirmaExcluir = async () => {
    if (tarefaId) {
      setCarregando(true); // Ativa o spinner
      try {
        await projeto5_backend.excluirTarefa(parseInt(tarefaId));
        await consultarTarefas(); // Atualiza a lista de tarefas
        await buscarTotais(); // Atualiza os totais
  ...
  async function alterar(id, categoria, descricao, urgente, concluida) {
    setCarregando(true); // Ativa o spinner
    try {
      await projeto5_backend.alterarTarefa(parseInt(id), categoria, descricao, urgente, concluida);
      await consultarTarefas(); // Atualiza a lista de tarefas
      await buscarTotais(); // Atualiza os totais
    } catch (error) {
